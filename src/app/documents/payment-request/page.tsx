...
const numberToWords = (value: number): string => {
  ...
  const readTriple = (num: number, full = false) => {
    ...
    if (t > 1) {
      ...
      else if (u > 0) w.push(ones[u]);
    } else if (t === 1) {
      w.push("mười");
      if (u === 5) w.push("lăm");
      else if (u > 0) w.push(ones[u]);
    } else if (u > 0) {
      if (h > 0 || full) w.push("lẻ");
      w.push(ones[u]);
    }
    return w.join(" ");
  };
  const groups = [
    { v: 1_000_000_000, l: "tỷ" },
    { v: 1_000_000, l: "triệu" },
    { v: 1_000, l: "nghìn" },
    { v: 1, l: "" },
  ];
  ...
  for (const g of groups) {
    const n = Math.floor(rem / g.v);
    rem %= g.v;
    if (n > 0) {
      res.push(readTriple(n, hasHigh), g.l);
      hasHigh = true;
    }
  }
...
function PaymentRequestContent() {
  ...
  const [docCode, setDocCode] = useState(
    () => `DNTT26-${String(Date.now()).slice(-5)}`,
  );
    ...
    <div className="min-h-full bg-[#eef2f8] p-4">
      <div className="mx-auto max-w-[860px]">
        ...
        <div className="print:hidden mb-4 border border-[#d8e0ee] bg-white p-4 text-sm space-y-3">
          ...
          <label className="grid grid-cols-[140px_1fr] items-center gap-2">
            <span>
              <b className="text-red-600">*</b> Kính gửi
            </span>
            <input
              value={toCompany}
          ...
          <label className="grid grid-cols-[140px_1fr] items-center gap-2">
            <span>
              <b className="text-red-600">*</b> Số tiền
            </span>
            <input
    ...
    >
      ...
      >
        <div style={{ fontSize: 11 }}>
          <div>
            <b>Đơn vị:</b> CTY TNHH GC CK INOX HP HỒNG PHÁT
          </div>
          <div>
            <b>Địa chỉ:</b> 198 Bình Long, KP. 12, P.Phú Thạnh, TP Hồ Chí Minh
          </div>
        </div>
      ...
      <div style={{ marginBottom: 4 }}>
        Họ và tên người đề nghị thanh toán: {blank(requestPerson, "260px")}
      </div>
      <div style={{ marginBottom: 4 }}>
        Bộ phận (Hoặc địa chỉ): {blank(department, "300px")}
      </div>
      <div style={{ marginBottom: 4 }}>
        Nội dung thanh toán:{" "}
        {content ? (
      ...
      >
        <div>
          ...
          <div style={{ marginTop: 70 }}>{creator || ""}</div>
        </div>
        <div>
